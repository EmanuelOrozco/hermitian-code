...
from .operators import assemble_systems, build_A_diagonal, build_H_hat
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Estructura de bandas fotónicas — formulación Hermitiana "
...
def build_config(args: argparse.Namespace) -> SimConfig:
    """Construye SimConfig desde argumentos CLI."""
    return SimConfig(
...
def compute_bands(config: SimConfig, logger) -> dict:
    """Calcula estructura de bandas a lo largo de Γ→X."""
    ...
    for ki, kx in enumerate(kx_path):
        h0, _, _ = assemble_systems(grid, kx, 0.0, gamma=0.0)
        if config.verbosity >= 2:
            from .validation import validate_system

            h_def, curl_err = validate_system(grid, kx)
        ...
        if config.verbosity >= 1:
            logger.info(
                f"  k[{ki}] k/π={k_norm[ki]:.3f}  ω: {summarize_frequencies(result.eigenvalues)}"
            )
    ...
    np.savetxt(out_dir / "bands.dat", np.column_stack([k_norm, bands]))
    ...
    with open(out_dir / "bands_meta.json", "w") as f:
...
def compute_loss(config: SimConfig, grid: YeeGrid, solver_cfg: SolverConfig, logger):
    """Caso con pérdidas: exacto vs perturbativo."""
...
def export_field(
    ...
    """Exporta perfil espacial de un modo."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.field_mode >= 0:
        args.field_kindex  # ensure attribute exists via parse
    ...
    try:
        ...
        if config.field_mode >= 0:
            export_field(
                config, grid, solver_cfg, config.field_mode, config.field_kindex, logger
            )
